    ...
    if data:
        if 'S' in data:
            ...
            for x in data['L']:
                ...
                if (override and list_sanitized_data is not None and list_sanitized_data != '') or not override:
            ...
            for k, v in data['M'].items():
                inline_sanitized_data, override = sanitify(v)
                if k:
                    if (override and inline_sanitized_data is not None) or not override:
                        inline_map[k] = inline_sanitized_data
            return inline_map, True
        else:
            return None, True
    ...
    with open('input.json', 'r') as r:
        ...
        for k, v in data.items():
            if k:
                ...
                if (override and sanitized_data is not None) or not override:
